...
export function OpenGamesScreen({ navigation }) {
  const [games, setGames] = useState([]);
  const [refreshing, setRefreshing] = useState(false);

  const loadGames = async () => {
    // In a real app you'd have a dedicated endpoint for open games
    // For now, show confirmed bookings that are open games
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Rejoignez un match ou organisez le vôtre</Text>
      </View>
      <View style={styles.comingSoon}>
        <Text style={styles.comingSoonIcon}>⚽</Text>
        <Text style={styles.comingSoonTitle}>Bientôt disponible !</Text>
        <Text style={styles.comingSoonText}>
          La fonctionnalité de matchs ouverts arrive bientôt.
          Réservez un terrain et invitez d'autres joueurs à rejoindre votre match.
        </Text>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  comingSoon: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: SPACING.xxl, gap: SPACING.md },
  comingSoonIcon: { fontSize: 64 },
  comingSoonTitle: { fontSize: FONTS.sizes.xl, fontWeight: '700', color: COLORS.textPrimary },
  comingSoonText: { fontSize: FONTS.sizes.base, color: COLORS.textSecondary, textAlign: 'center', lineHeight: 24 },
});

export default OpenGamesScreen;
...
export function BookingDetailScreen({ navigation, route }) {
  ...
  useEffect(() => {
    bookingsAPI.getOne(bookingId).then(res => setBooking(res.data.booking)).catch(() => navigation.goBack());
  }, []);

  if (!booking) return <View style={styles.loading}><Text>Chargement...</Text></View>;
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.card}>
          ...
          <View style={styles.row}><Text style={styles.label}>Durée:</Text><Text style={styles.value}>{booking.duration_minutes} minutes</Text></View>
          <View style={styles.row}><Text style={styles.label}>Paiement:</Text><Text style={styles.value}>{booking.payment_method}</Text></View>
          <View style={styles.row}><Text style={styles.label}>Montant:</Text><Text style={[styles.value, { color: COLORS.primary, fontWeight: '700' }]}>{parseFloat(booking.total_price).toFixed(2)} MAD</Text></View>
...
const styles = StyleSheet.create({
  ...
  ref: { fontSize: FONTS.sizes.xs, color: COLORS.textLight, textAlign: 'center', fontFamily: 'monospace' },
});
